...
import CreateModal from "../Crud/create.modal";
import UpdateModal from "@/components/Crud/update.modal";
...
function TableThree(props: IProps) {
  const { blogs } = props;
  ...
  return (
    <div className="rounded-[10px] border border-stroke bg-white p-0  shadow-1 dark:border-dark-3 dark:bg-gray-dark dark:shadow-card sm:p-7.5">
      <div>
        <Button
          className="mb-3 flex w-full max-w-[90px] cursor-pointer items-center justify-center gap-2 rounded-lg bg-primary p-2 font-medium text-white transition hover:bg-opacity-90"
          variant="primary"
      ...
      <div className="max-w-full overflow-x-auto">
        <table className="w-full table-auto">
          <thead>
            <tr className="bg-[#F7F9FC] text-left dark:bg-dark-2">
              <th className="min-w-[50px] px-4 py-4 font-medium text-dark dark:text-white ">
                Index
              </th>
          ...
          <tbody>
            {validBlogs.map((packageItem, index) => (
              <tr key={index} className="hover:bg-[#0f1825]">
                <td
                  className={` px-4 py-4 dark:border-dark-3 xl:pl-7.5 ${index === validBlogs.length - 1 ? "border-b-0" : "border-b"}`}
                >
                  <div className="flex flex-shrink-0 justify-center rounded-full border border-white">
                    {index + 1}
                  </div>
                </td>
                <td
                  className={`border-[#eee] px-4 py-4 dark:border-dark-3 xl:pl-7.5 ${index === validBlogs.length - 1 ? "border-b-0" : "border-b"}`}
                >
                  <div className="flex h-[100px] w-[100px] items-center justify-center overflow-hidden">
                    <img
                      className="h-full w-full object-contain" // "object-contain" sẽ đảm bảo ảnh giữ nguyên tỷ lệ gốc
                ...
                >
                  <h5 className="text-dark dark:text-white">
                    {packageItem.seriesName}
                  </h5>
                </td>
                <td
                  className={` border-[#eee] px-4 py-4 dark:border-dark-3 ${index === blogs.length - 1 ? "border-b-0" : "border-b"}`}
                >
                  <p className="max-h-20 overflow-auto text-dark dark:text-white">
                ...
                {/*</td>*/}
                <td
                  className={`border-[#eee] px-4 py-4 dark:border-dark-3 xl:pr-7.5 ${index === blogs.length - 1 ? "border-b-0" : "border-b"}`}
                >
                  <div className="flex items-center justify-end space-x-3.5">
                    ...
                    <button
                      className="hover:text-primary"
                      // onClick={() => handleDeleteBlog(packageItem.id)}
                    >
                      <svg
                        className="fill-current"
                        width="20"
        ...
        <Link className="hover:text-primary" href={`/detail/`}>
          <svg
            className="fill-current"
            width="20"
            height="20"
